...
public class CategoriesController {
    ...
    public ResponseEntity<UUID> deleteCategory(@PathVariable UUID categoryId, Authentication authentication) {
        UUID userId = (UUID) authentication.getPrincipal();
        return ResponseEntity.ok(service.deleteCategory(categoryId, userId));
    }

    @PatchMapping("/categories/{categoryId}")
    public ResponseEntity<Category> updateCategory(@PathVariable UUID categoryId, @RequestBody CreateCategoryRequest category, Authentication authentication) {
        UUID userId = (UUID) authentication.getPrincipal();
